...
export default {
      ...
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: 'Get all the latest movies in theaters and online'
        },
        {
          hid: 'keywords',
          name: 'keywords',
  ...
  methods: {
    async getMovies() {
      const result = await axios.get(`https://api.themoviedb.org/3/movie/now_playing?api_key=d2b3cf358fb2d8f15bce8546ff06eb5b&language=en-US&page=1`)
    ...
    async searchMovies() {
      ...
      const result = await axios.get(`https://api.themoviedb.org/3/search/movie?api_key=d2b3cf358fb2d8f15bce8546ff06eb5b&language=en-US&page=1&query=${this.searchInput}`)
      result.data.results.forEach((movie) => {
